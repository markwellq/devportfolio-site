        ...
        <div className="min-w-0 font-mono text-sm text-main">
            <div className="mb-8">
                <p className="text-sm sm:text-base">
                    <span className="font-semibold">denoqcore@site</span>{" "}
            ...
            <div className="mb-6 hidden grid-cols-[150px_minmax(0,1fr)] md:grid">
                ...
                <span>{t("columnTechnologies")}</span>
            </div>
            ...
            <div className="space-y-7">
                {STACK_CATEGORIES.map((category) => (
                    <div
                        key={category.key}
                        className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-[150px_minmax(0,1fr)] md:gap-5"
                    >
                                ...
                                {category.items.map((tech) => {
                                    ...
                                    if (!match) {
                                        return (
                                            <span
                                                key={tech}
                                                className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-tinted px-3 py-2 text-xs text-main sm:px-4 sm:text-sm"
                                            >
                                    ...
                                    return (
                                        <span
                                            key={tech}
                                            className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-tinted px-3 py-2 text-xs text-main transition-colors duration-200 hover:bg-tint/10 sm:px-4 sm:text-sm cursor-pointer"
                                        >
                                            <Icon size={18} color={match.color} />
                                            <span>{tech}</span>
                                        </span>
                                    );
                                })}
            ...
            <div className="space-y-3 text-xs sm:text-sm">
                <p className="flex items-start gap-3 text-green-500">
                    <CheckIcon size={18} weight="bold" className="mt-0.5 shrink-0" />
                    <span>
                        {t("loaded", { count: resolvedCount, total: allTechs.length, percentage })}
                    </span>
                </p>

                <p className="flex items-center gap-3 text-main">
                    <FlagIcon size={18} className="shrink-0" />
                    <span>{t("renderTime", { ms: renderMs === null ? "…" : `${renderMs}ms` })}</span>
                </p>
            </div>
        </div>
    );
